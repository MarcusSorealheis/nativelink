...
Before this enhancement, connecting to gRPC endpoints using TLS in Nativelink required manual configuration of [certificate files](https://en.wikipedia.org/wiki/Certificate_authority) - specifically providing CA certificate files, certificate files, and key files. This created significant friction for teams using managed certificates or deploying in environments where distributing secrets across workers was either impractical or posed security concerns.

Consider the previous [`ClientTlsConfig`](https://nativelink.com/docs/reference/nativelink-config/#clienttlsconfig) structure:
...
Every TLS connection definition. Now, Nativelink can automatically detect certificates from the host. For teams leveraging cloud-managed certificates or certificate authorities, this approach created unnecessary operational overhead and potential security vulnerabilities.
